#!/usr/bin/env bash
# Claude Code status line: RGB gradient, dynamic emoji, cost, code velocity, effort level
...
# ── Effort level ──
if [ -n "$effort" ]; then
  e_lower=$(echo "$effort" | tr '[:upper:]' '[:lower:]')
  case "$e_lower" in
    max)  e_color="$RED"    e_icon="🔥";;
    xhigh) e_color="$YELLOW" e_icon="⚡";;
    high) e_color="$GREEN"  e_icon="▲";;
    medium) e_color="$BLUE"  e_icon="◆";;
    low)  e_color="$CYAN"   e_icon="▼";;
    *)    e_color="$DIM"    e_icon="?";;
  esac
  effort_part="${e_color}${e_icon} ${e_lower}${RESET}"
fi
...
[ -n "$effort" ] && out="${out} ${DIM}|${RESET} ${effort_part}"
