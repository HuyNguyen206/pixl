...
<template>
    <div v-if="post.profile" class="bg-pixl-light/20 p-4 transition duration-150 ease-in-out">

        <Form method="POST" :action="route('replies.store', [post.profile, post])"
              class="mt-4 grow"
              reset-on-success
              #default="{ errors }"
        ...
        >
            ...
            <div class="flex gap-2 justify-start items-center">
                <textarea cols="30" rows="5" name="content" id="content" class="border-none w-full resize-none"
                          :placeholder="`Reply to ${post.profile.handle}'s post`"></textarea>
            </div>
            <div v-if="errors.content" v-text='errors.content' class="text-red-400"></div>
            ...
            <div class="flex justify-between">
                ...
                <input type="submit" class="bg-pixl border border-transparent px-4 py-0.5 text-pixl-dark" value="Reply">
            </div>
        </Form>
    </div>
</template>
